...
async function loadTags(){library=null;tagFilter.value='';try{library=await BibleTags.load(root)}catch{message('Les tags ne peuvent pas être enregistrés dans ce navigateur.')}updateTagFilter();}
...
function card(item,path,searchResult=false){
  ...
  button.onclick=async()=>{if(handle.kind==='directory'){trail=[...path,handle];ui.search.value='';tagFilter.value='';await render()}else await openFile(handle)};
...
async function searchEverywhere(term,token){
  const found=[],selected=tagFilter.value;
  const walk=async(dir,path)=>{for(const item of await sorted(dir)){
    ...
    const matchText=!term||item.name.toLocaleLowerCase('fr').includes(term)||tags.some(tag=>tag.includes(term));
    if(matchText&&(!selected||tags.includes(selected)))found.push({item,path});
